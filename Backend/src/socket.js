...
const getData = async () => {
    ...
    let curMax = maxBlockNumber;
    if (scanBlockNumber != 0 && scanBlockNumber <= curMax) {
        console.log('scan block number', scanBlockNumber);
        try {
            await SingleMintOnSale_monitor(scanBlockNumber, curMax);
            await DestroySale_monitor(scanBlockNumber, curMax);
            await PlaceBid_monitor(scanBlockNumber, curMax);
            await AcceptBid_monitor(scanBlockNumber, curMax);
            await BuyNow_monitor(scanBlockNumber, curMax);
            await EndBid_monitor(scanBlockNumber, curMax);
            await TransferNFT_monitor(scanBlockNumber, curMax);
            await BurnNFT_monitor(scanBlockNumber, curMax);
            await ChangePrice_monitor(scanBlockNumber, curMax);
            await BatchMintOnSale_monitor(scanBlockNumber, curMax);
            await BatchEndAuction_monitor(scanBlockNumber, curMax);
            scanBlockNumber = curMax + 1;
        } catch (e) {
                ...
                if (compareObjects(SingleMintOnSaleTemp, objTemp) === false) {
                    ...
                    Item.findByIdAndUpdate(tokenHash, param).then((data) => {
                        ...
                        let descriptionStr = data.auctionPeriod === 0 ?
                            "An instant sale is opened on " + data.name + " with price " + data.auctionPrice
