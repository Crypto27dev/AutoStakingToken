...
import { fetchAuthorDetail } from "../../store/actions/thunks";
...
const Author = ({ authorId }) => {
  ...
  const authorDetail = useSelector(selectors.authorDetail);
  const author = useMemo(() => {
    return authorDetail.data ? authorDetail.data : {};
  }, [authorDetail])

  useEffect(() => {
    setNavImage((author && author.banner) ? api.imgUrl + author.banner : api.rootUrl + "/img/background/1.jpg");
    setAvatar(getAvatar(author));
  }, [author]);

  useEffect(() => {
    dispatch(fetchAuthorDetail(authorId));
  }, [dispatch, authorId]);
  ...
  return (
    <div>
              ...
              <div className="de-flex-col">
                <div className="profile_avatar">
                  ...
                  <div className="profile_name">
                    <h4>
                      {author.username} { currentUser._id === author._id && ("(You)")}
      ...
      <section className='container no-top'>
        ...
        {openMenu && author && author._id && (
          <div id='zero1' className='onStep fadeIn'>
            <ColumnNewRedux showLoadMore={true} authorId={authorId} activeIndex={0} />
          </div>
        )}
        {openMenu1 && author && author._id && (
          <div id='zero2' className='onStep fadeIn'>
            <ColumnNewRedux showLoadMore={true} authorId={authorId} activeIndex={1}/>
          </div>
        )}
        {openMenu2 && author && author._id && (
          <div id='zero3' className='onStep fadeIn'>
            <ColumnNewRedux showLoadMore={true} authorId={authorId} activeIndex={2}/>
          </div>
        )}
        {openMenu3 && author && author._id && (
          <div id='zero4' className='onStep fadeIn'>
            <ColumnNewRedux showLoadMore={true} authorId={authorId} activeIndex={3}/>
          </div>
        )}
      </section>
